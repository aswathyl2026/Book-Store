...
const navigate=useNavigate()
  ...
  const handleSearch=()=>{
    if(!searchKey){
      toast.warning("enter an input")
    }else if(!sessionStorage.getItem("token")){
      toast.warning("Plz login to continue...")
      setTimeout(() => {
        navigate('/login')
      }, 2000);
      ...
      <div style={{ height: '500px' }} className="flex flex-col items-center justify-center bg-[url(/land3.png)] bg-cover text-white">
        <div style={{ height: '500px', backgroundColor: 'rgba(0,0,0,0.5)' }} className=" flex flex-col items-center justify-center w-full ">
          ...
          <div className="mt-9 flex items-center">
            <input onChange={(e)=>setSearchKey(e.target.value)} type="text" placeholder="Search A Book" className="bg-white p-2 w-100 text-black rounded-3xl" />
